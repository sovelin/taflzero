...
def make_random_batch(batch_size: int, actions: int = 4840) -> tuple[torch.Tensor, ...]:
    x = torch.rand(batch_size, 6, 11, 11, dtype=torch.float32)

    # Build legal mask with at least one legal action per sample.
    legal_mask = torch.rand(batch_size, actions) < 0.12
    legal_mask[:, 0] = True

    # Simulated MCTS visit counts over legal actions.
    visit_counts = torch.randint(0, 200, (batch_size, actions), dtype=torch.float32)
    ...
    value_target = torch.empty(batch_size, 1).uniform_(-1.0, 1.0)
...
def main() -> None:
    ...
    x, legal_mask, pi_target, value_target = make_random_batch(batch_size=8)
