...
fn evaluate_attackers_mobility(board: &Board) -> i32 {
	...
	for idx in 0..board.attackers_count as usize {
		let moves = possible_moves_count(board, board.attackers[idx]).min(limit);
		result += ATTACKERS_MOBILITY_SCORES[moves];
	}
...
#[inline]
fn evaluate_attackers_distance_to_king(board: &Board, king_sq: Square) -> i32 {
	let mut result = 0;
	let limit = ATTACKERS_DISTANCE_TO_KING_SCORES.len() - 1;

	for idx in 0..board.attackers_count as usize {
		let sq = board.attackers[idx];
		let distance = PRECOMPUTED.manhattan_distance[sq][king_sq].min(limit);
		result += ATTACKERS_DISTANCE_TO_KING_SCORES[distance];
	}
...
#[inline]
fn evaluate_defenders_distance_to_king(board: &Board, king_sq: Square) -> i32 {
	let mut result = 0;
	let limit = DEFENDERS_DISTANCE_TO_KING_SCORES.len() - 1;

	for idx in 0..board.defenders_count as usize {
		let sq = board.defenders[idx];
		let distance = PRECOMPUTED.manhattan_distance[sq][king_sq].min(limit);
		result += DEFENDERS_DISTANCE_TO_KING_SCORES[distance];
	}
...
pub fn evaluate(board: &Board) -> i32 {
	...
	let defenders_count = board.defenders_count as i32;
	let attackers_count = board.attackers_count as i32;

	let mut score = 0;
	score += defenders_count * DEFENDER_WEIGHT;
	score -= attackers_count * ATTACKER_WEIGHT;

	score += PSQT_KING[king_sq] as i32;

	score += evaluate_defenders_mobility(board, king_sq);
	score -= evaluate_attackers_mobility(board);

	score += evaluate_defenders_distance_to_king(board, king_sq);
	score -= evaluate_attackers_distance_to_king(board, king_sq);

	for idx in 0..board.defenders_count as usize {
		let sq = board.defenders[idx];
		score += PSQT_DEF[sq] as i32;
	}

	for idx in 0..board.attackers_count as usize {
		let sq = board.attackers[idx];
		score -= PSQT_ATK[sq] as i32;
	}

	let mut surroundings = 0usize;
	for &sq in PRECOMPUTED.vertical_horizontal_neighbors[king_sq].iter() {
		if board.board[sq] == Piece::ATTACKER {
			surroundings += 1;
		}
	}

	let bonus_idx = surroundings.min(KING_SURROUNDING_BONUSES.len() - 1);
	score += KING_SURROUNDING_BONUSES[bonus_idx];

	sided_evaluation(score, board.side_to_move)
}
