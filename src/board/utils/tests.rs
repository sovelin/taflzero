    ...
    mod neighbors {
        ...
        mod mass_neighbor_retrieval {
            ...
            fn vertical_horizontal_center() {
                let sibs = get_vertical_horizontal_neighbors(get_square_from_algebraic("e6"), 11);
                expect_exactly_neighbors(&["e7", "e5", "d6", "f6"], &sibs);
            }

            #[test]
            fn vertical_horizontal_edge() {
                let sibs = get_vertical_horizontal_neighbors(get_square_from_algebraic("a6"), 11);
                expect_exactly_neighbors(&["a7", "a5", "b6"], &sibs);
            }

            #[test]
            fn vertical_horizontal_corner() {
                let sibs = get_vertical_horizontal_neighbors(get_square_from_algebraic("a1"), 11);
                expect_exactly_neighbors(&["a2", "b1"], &sibs);
            }

            #[test]
            fn all_neighbors_center() {
                let sibs = get_all_neighbors(get_square_from_algebraic("e6"), 11);
                expect_exactly_neighbors(&["e7", "e5", "d6", "f6", "d7", "f7", "d5", "f5"], &sibs);
            }

            #[test]
            fn all_neighbors_edge() {
                let sibs = get_all_neighbors(get_square_from_algebraic("a6"), 11);
                expect_exactly_neighbors(&["a7", "a5", "b6", "b7", "b5"], &sibs);
            }

            #[test]
            fn all_neighbors_corner() {
                let sibs = get_all_neighbors(get_square_from_algebraic("a1"), 11);
